...
const AuthProvider = (props) => {
  ...
  useEffect(() => {
    ...
    if (!isValidPrivateKey(pk)) {
      ...
      console.log(
        "Resetting the localstorage for security reasons, login again please :)"
      );
      localStorage.clear();
      return false;
    }
    ...
    if (newWallet.address !== expectedAddress) {
      ...
      console.log(
        "Local storage address(expected):\n",
    ...
    universalLogin(pk, authorityType)
      ...
      .catch((err) => {
        console.log(
          "Logging in from saved credentials failed, Resetting localstorage !!"
        );
        localStorage.clear();
        console.log(err);
      });
